...
$statement = T(
  ...
  sub {
    print Dumper(['statement (first)', \@_]) if $ENV{DEBUG};
    \@_
  }
);
...
my $statements = T(
  ...
  sub {
    print Dumper(['all statements', \@_]) if $ENV{DEBUG};
...
my $entire_input = T(
  ...
  sub { 
    print Data::Dumper->Dump([\@_],['*entire_input']) if $ENV{DEBUG}; 
    $_[0]
  }
);
