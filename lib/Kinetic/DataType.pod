# $Id: DateTime.pm 2507 2006-01-11 01:12:14Z theory $
...
Kinetic::DataType - Complex data types for TKP

=head1 Description

The C<Kinetic::DataType> name space is set aside for the creation of complex
...
L<Kinetic::Meta::DataTypes|Kinetic::Meta::DataTypes>.
...
L<Kinetic::Meta::DataTypes|Kinetic::Meta::DataTypes> are I<always> loaded by
TKP, since it uses them for its own classes, the complex data types in the
C<Kinetic::DataType> name space tend to be loaded only as needed by the
...
inherits from L<Kinetic|Kinetic> and is implemented using
L<Kinetic::Meta|Kinetic::Meta> is already a data type. Its C<key> attribute
will name is as an attribute, and nothing further needs to be done.
...
Kinetic::Util::Exception::Fatal::Invalid exception--most easily done with the
convenient C<throw_invalid()> function importable from
L<Kinetic::Util::Exceptions|Kinetic::Util::Exceptions>. Note that the way
...
L<Kinetic::Util::Language|Kinetic::Util::Language>.
...
should simply be added directly to C<Kinetic::Util::Language::en>, like so:
...
L<Kinetic::Build::Schema::DB::SQLite|Kinetic::Build::Schema::DB::SQLite>
...
F<t/store/TEST/Kinetic/Store/DB.pm> for some examples).
...
already a data type in TKP, used by
L<Kinetic::VersionInfo|Kinetic::VersionInfo>. But it makes a nice example, so
we'll take a look at it here. Once again, the data type declaration simply
requires a the use of L<Kinetic::Meta::Type|Kinetic::Meta::Type>:
...
C<Kinetic::Meta::Type> will recognize it as such, and build a validating
...
L<Kinetic::DataTypee::Duration|Kinetic::DataTypee::Duration>, or you need to
...
add the new class to the Kinetic::DataType name space.

For example, say that we wanted to add a L<DateTime|DateTime> data type. In
fact, TKP includes a such a data type,
L<Kinetic::DataType::DateTime|Kinetic::DataType::DateTime>, but it makes a
...
defined in the Kinetic::DataType name space are not automatically loaded, they
...
L<Kinetic::Build::Schema::DB::Pg|Kinetic::Build::Schema::DB::Pg> and
L<Kinetic::Build::Schema::DB::SQLite|Kinetic::Build::Schema::DB::SQLite>. It
...
L<Kinetic::DataType::Duration|Kinetic::DataType::Duration> is a case in point.
...
To accommodate this, there is yet another C<Kinetic::Meta::Type> parameter for
...
Here's how C<Kinetic::DataType::Duration> uses it:
...
C<Kinetic::Meta::Type> uses C<raw>, instead; and if C<raw> isn't specified, it
...
Because C<STORE_CLASS> is a constant, this is actually very efficient.
...
L<Kinetic::Build::Schema::DB::Pg|Kinetic::Build::Schema::DB::Pg>:
...
underscores. To get it in use, change the value stored in the C<%types>
hash to use the new domain:

  version => 'VERSION',

It's possible to enforce such constraints in SQLite, as well, using triggers
and its C<REGEXP> keyword (which is will use the C<regexp()> SQLite function
...
be created, the name of the domain, and the right-hand side of a SQL C<WHERE>
expression that will trigger an exception when it evaluates to true. Our
