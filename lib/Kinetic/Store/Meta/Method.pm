package Kinetic::Store::Meta::Method;

# $Id$

use strict;

use version;
our $VERSION = version->new('0.0.2');

use base 'Class::Meta::Method';

=head1 Name

Kinetic::Store::Meta::Method - Kinetic object method introspection

=head1 Synopsis

  # Assuming MyThingy was generated by Kinetic::Store::Meta.
  my $class  = MyThingy->my_class;
  my $thingy = MyThingy->new;

  print "\nMethods:\n";
  for my $meth ($class->methods) {
      print "  o ", $meth->name, $/;
      $meth->call($thingy);
  }

=head1 Description

This class inherits from L<Class::Meta::Method|Class::Meta::Method> to provide
method metadata for Kinetic classes. See the L<Class::Meta|Class:Meta>
documentation for details on meta classes. See the L<"Instance Interface">
section for the attributes added to Kinetic::Store::Meta::Method in addition to those
defined by Class::Meta::Method.

=cut

##############################################################################
# Instance Methods.
##############################################################################

=head1 Instance Interface

=head2 Accessor Methods

=head3 delegates_to

  my $delegates_to = $meth->delegates_to;

If the method transparently delegates to an object of another class, this
method will return the Kinetic::Store::Meta::Class object describing that class. This
method is implicitly set by Kinetic::Store::Meta for classes that either extend or
mediate another class, or are a type of another class. In those cases,
Kinetic::Store::Meta will create extra methods to delegate to the methods of the
referenced or extended classes, and those methods will have their
C<delegates_to> methods set accordingly.

=cut

sub delegates_to { shift->{delegates_to} }

##############################################################################

=head3 acts_as

  my $acts_as = $meth->acts_as;

If C<delegates_to()> returns a Kinetic::Store::Meta::Class object representing the
class of object to which the method delegates, C<acts_as()> returns the
Kinetic::Store::Meta::Method object to which this method corresponds. That is, this
method I<acts as> the method returned here.

=cut

sub acts_as { shift->{acts_as} }

1;
__END__

##############################################################################

=head1 Copyright and License

Copyright (c) 2004-2006 Kineticode, Inc. <info@kineticode.com>

This module is free software; you can redistribute it and/or modify it under the
same terms as Perl itself.

=cut
