# $Id: DB.pm 1979 2005-08-16 01:15:04Z curtis $
...
Kinetic::Store::DB::Overview - Developer's overview of the Kinetic::Store::DB
...
complicated SQL to manage these relationships, a view is created which makes
...
activates triggers or calls rules which take the C<INSERT> or C<UPDATE> and
creates or modifies the appropriate records.  Thus, the C<Kinetic::Store::DB>
merely needs to determine the name of the view and the correct column names for
said view.
...
Then, it determines the name of the view which represents this class:

  my $view = $kinetic_class->key;

If a Kinetic class contains (or "references") other classes, we first iterate
over those other classes and save them by recursively calling the C<&_save>
method.
...
Every Kinetic class has a set of attributes.  Those are either direct
attributes or referenced attributes.  The "referenced" attributes refer to
contained objects.  Once the contained objects have been saved, we need to save
the actual object.  This is done by determining the names of the view's columns
for each attribute and its value.
...
If the object has an C<id> (the C<id> is for internal use only -- other classes
should never see this), we know it already exists in the database so we call
the C<&_update> method.  Otherwise, we call the C<&_insert> method.
...
=head2 C<< $kinetic_object->search( @params ) >>

The C<< $object->search >> method is where the real magic happens.  The 
C<< $object->search_guids >>, C<< $object->lookup >> and C<< $object->count >>
methods are closely related, so the following description applies to them also.

  my $iter = $kinetic_object->search( @search_params );

This returns a L<Kinetic::Util::Iterator|Kinetic::Util::Iterator> object
containing all objects that match the search params. See
L<Kinetic::Store|Kinetic::Store> for detailed information about search params.

First we'll give a general overview of how C<&search> method works and then
we'll go into detail about the various parts.

As usual, C<&search> calls C<&_search> internally.  The latter method
determines what type of search we are working with.  This "type" of search
is either C<CODE> or C<STRING>.  A C<CODE> search looks like this:

 $object->search( \@parameters, \%constraints );

A string search is similar:

 $object->search( STRING => $parameters, \@constraints );
...
The "parameters" correspond to what one would find in a where clause and the
"contraints" are things such as "limit", "offset" or other things that affect
the behavior of how results are returned. 

B<Note>:  If no parameters are supplied, every object for a given class is
returned.  Constraints are optional.  However, if constraints are desired for
string searches, you must pass an empty string search, first:

  $object->search( STRING => '', \@constraints );
...
Instead, the the string is lexed into a series of tokens the search parser
understands.  More on this later.

As of this writing, determining which type of search is done by examining the
first parameter to search in the C<&_set_search_type> method.  If it's
I<STRING>, we have a string search.  Otherwise, it's assumed to be a code
search.  No other search methods are supported at this time.  If we have a
I<STRING> search type, we check to see if there are any constraints and those
are converted into a hashref matching the I<CODE> search type hashref.

Once the search type is determined, we set the search data via a method
...
the search and convert results back into Kinetic objects.  Because this method
...
Then, we use the view's columns and the search parameters and constraints to
...
=head2 C<< $store->_set_search_type( \@search ) >>

This method is straight forward.  As mentioned earlier, if the first argument
is I<STRING>, then we have a string search and the third C<@search> argument
and beyond are considered to be constraints and those are converted into a
hashref that matches the I<CODE> hashref.
...
=head2 C<< $store->_get_select_sql_and_bind_params( $columns, \@search ) >>

Once again, we have a fairly straightforward method.  When we get here, we
fetch the name of the view, we build a where clause from the
C<&_make_where_clause> method, add constraints, if any, and return the
sql and bind params.  The relevant code looks like this:
...
=head2 C<< $self->_make_where_clause( \@search ) >>

This is where the start of the real magic happens.  This method returns a where
clause and an arrayref of any appropriate bind params for that where clause.
Returns an empty string and an empty array ref (the bind params) if no where
clause can be generated.
...
respectively.  Each takes the appropriate search parameters (either the Perl
code or the search string) and returns a virtually identical stream of tokens.
You can see exact details of their behavior in C<t/lexer/code.t> and 
C<t/lexer/string.t>.
...
Both of those searches, when lexed, will produce the following series of
tokens:
...
turn a I<CODE> search into a I<STRING> search.  This may seem a bit strange,
but what it gives us is this:

    my $intermediate_representation = parse( $stream, $store );

The parse method is exported by C<Kinetic::Store::Parser::DB>.  This parser
...
list of C<Kinetic::Store::Search> objects (see the documentation for details)
and groups of said objects (see C<Kinetic::Store::Parser::DB> for more
information on this).

By doing it this way, we can have multiple search interfaces and one parser
for all searches.  This simplifies the code tremendously.  Further, if other
data stores (such as LDAP) are implemented in the future they can use the same
IR to handle their searches.  Nothing up to this point is specific to SQL.

Now that we have the IR, we convert it to a where clause with the
...
This method essentially walks through the IR array and for each element
in the array, builds up a "where snippet" and pushes it onto a stack and pushes
the associated bind param onto a stack, too.  These "snippets" resemble things
such as:
...
Note that some columns have the column and its value lower-cased.  This is
...
Now that we have the where clause built up, we go back up to the
...
representing the results of a given C<search>.  Some methods require all
...
For a given hashref returned by C<$sth-E<gt>fetchrow_hashref>, the
