...
object containing all objects that match the search parameters. See
L<Kinetic::Store|Kinetic::Store> for detailed information about search
parameters.
...
As usual, C<search> calls C<_search> internally. The latter method determines
what type of search we are working with. This "type" of search is either
C<CODE> or C<STRING>. A C<CODE> search looks like this:
...
the "contraints" are things such as "limit", "offset" or other things that
...
As of this writing, C<_search> determines the type of search by calling the
C<&_set_search_type> method to examine the first parameter. If it's "STRING",
we have a string search. Otherwise, it's assumed to be a code search. No other
search methods are supported at this time. If we have a I<STRING> search type,
we check to see if there are any constraints and convert them into a hashref
matching the I<CODE> search type hashref.
...
I<STRING>, then we have a string search and the third argument and beyond are
...
This aproach allows us to have multiple search interfaces and one parser for
...
object representing the results of a given C<search>. Some methods require all
