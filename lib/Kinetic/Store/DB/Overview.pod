...
create or modify the appropriate records.  Thus, C<Kinetic::Store::DB>
...
Every Kinetic class has a set of attributes, either direct
...
If the object has an C<id> attribute (the C<id> is for internal use only -- other classes
...
This method returns a L<Kinetic::Util::Iterator|Kinetic::Util::Iterator> object
containing all objects that match the search parameters. See
L<Kinetic::Store|Kinetic::Store> for detailed information about search parameters.

First we'll give a general overview of how the C<search> method works and then
...
The "parameters" correspond to what one would find in a C<WHERE> clause and the
"contraints" are things such as "limit", "offset" or other things that affect
the how results are returned.
...
As of this writing, C<_search> determines the type of search by calling
the C<&_set_search_type> method to examine the
first parameter.  If it's
"STRING", we have a string search.  Otherwise, it's assumed to be a code
search.  No other search methods are supported at this time.  If we have a
I<STRING> search type, we check to see if there are any constraints and
convert them into a hashref matching the I<CODE> search type hashref.
...
This method is straightforward.  As mentioned earlier, if the first argument
is I<STRING>, then we have a string search and the third argument
and beyond are considered to be constraints. The constrains are converted into a
...
C<&_make_where_clause> method, add any constraints, and return the
SQL and bind parameters.  The relevant code looks like this:
...
This is where the start of the real magic happens.  This method returns a C<WHERE>
clause and an arrayref of any appropriate bind parameters for that C<where> clause.
Returns an empty string and an empty array ref (the bind parameters) if no C<WHERE>
...
said objects (see L<Kinetic::Store::Parser::DB|Kinetic::Store::Parser::DB> for
more information on this).

This aproach allows us to have multiple search interfaces and one parser
for all searches.  This simplifies the code tremendously.  Further, if other
data stores (such as LDAP or XML) are implemented in the future, they can use the same
...
method usually returns a L<Kinetic::Util::Iterator|Kinetic::Util::Iterator> object
