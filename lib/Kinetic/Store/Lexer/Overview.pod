# $Id: Lexer.pm 1979 2005-08-16 01:15:04Z curtis $
...
have two main ways of searching. If writing Perl code which directly uses the
...
security hole. Instead, we use code and string lexers which will convert each
type of search into virtually identical token streams. By doing this, we can
use a single parser, L<Kinetic::Store::Parser|Kinetic::Store::Parser> to
...
identical lexer streams and the parser will be able to understand it.
...
To do this, we first need to be able to unambiguously identify each and every
component of a string search. We can do this by either creating exact strings
which match a component (such as "undef") or regular expressions which match
...
B<Note>:  In the above code, C<$QUOTED> and C<$NUM> are from C<Regexp::Common>.
...
and error.  In the above example, wee need the C<KEYWORD> token to come after
the C<VALUE> token lest any text in a keyword be extracted from quoted strings
prior to identified as values.

C<COMPARE> and C<KEYWORD> must, in turn, come before C<IDENTIFIER> or else all
comparison terms or keywords would be lexed as identifiers.

Note that even though whitespace is identified (the lexer must successfully be
able to identify everything in the text), it gets discarded as whitespace is
...
L<Kinetic::HOP::Lexer|Kinetic::HOP::Lexer> for details on how the actual
lexing and discarding of whitespace happens.
...
function, C<string_lexer_stream>. This function takes a valid code search and
...
merely a string, the C<$type> is I<standard>. The standard lexer is called and
it pushes to tokens onto the stack:

  [ 'IDENTIFIER', 'name' ]
  [ 'OP',           '=>' ]

Then it examines more of the seach request. If what follows was a string, it
would merely push appropriate tokens onto the stack  For example, with the
...
That generates the following stack:
...
The C<COMPARE> and C<VALUE> tokens are then pushed onto the stack, giving us
this:
