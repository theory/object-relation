...
Kinetic::Store::Parser::Overview - Developer's overview of 
Kinetic::Store::Parser

=head1 Synopsis

See L<Kinetic::Store::Parser|Kinetic::Store::Parser>.
...
C<Kinetic::Store::Parser> class operates.

=head1 Grammar

The C<Kinetic::Store::Parser> uses the Parser module described in "Higher
Order Perl" by Mark-Jason Dominus to transform a series of tokens into an
intermediate representation the C<Kinetic::Store> subclasses can use to
initiate searches against a datastore.
...
the IR created by C<Kinetic::Store::Parser> and how it's created.
