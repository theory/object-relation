...
L<Kinetic::Store::Parser|Kinetic::Store::Parser> class operates.

=head1 Grammar

L<Kinetic::Store::Parser|Kinetic::Store::Parser> uses the Parser module
...
L<Kinetic::Store::Handle|Kinetic::Store::Handle> subclasses can use to initiate searches
against a datastore.
...
by L<Kinetic::Store::Parser|Kinetic::Store::Parser> and how it's created.
...
L<Kinetic::Store::Lexer::String|Kinetic::Store::Lexer::String> both combine
the tokenizing and lexing into a single step. See
L<Kinetic::Store::Lexer::Overview|Kinetic::Store::Lexer::Overview> for
details.

This module has only one public function, C<parse>, which expects a stream as
returned by a lexer and a Kinetic store object:

  my $stream = code_lexer_stream($search_request);
  my $ir     = parse($stream, $store);

The C<parse()> function will then either throw a
C<Kinetic::Store::Exception::Fatal::Search> exception or return an IR.
...
L<Kinetic::Store::DataType::DateTime::Incomplete|Kinetic::Store::DataType::DateTime::Incomplete> string and, if
...
L<Kinetic::Store::Search|Kinetic::Store::Search> object.
...
L<Kinetic::Store::Handle|Kinetic::Store::Handle> object that was passed in to validate that
the search can actually be performed on the Kinetic class on which we are
currently searching.
