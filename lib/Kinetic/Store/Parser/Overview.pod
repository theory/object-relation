...
... and a Kinetic store object:

  my $ir = parse($stream, $store);

It then will either throw a C<Kinetic::Util::Exception::Fatal::Search>
exception or return an IR.

The store object is then used in the final step of building the IR. The store
