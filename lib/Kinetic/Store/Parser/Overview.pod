...
Kinetic::Store::Parser::Overview - Developer's overview of Kinetic::Store::Parser
...
by C<Kinetic::Store::Parser> and how it's created.
...
value ("bar"), and an C<$rbracket>, ("]"). The transformation is returning a
new token with the string "BETWEEN" and two normalized values.

As of this writing, the normalized value function merely examines the values
to see if they match a C<Kinetic::DateTime::Incomplete> string and, if they
do, return an object representing them.
...
actually be performed on the Kinetic class on which we are currently
searching.
