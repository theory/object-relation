...
This module has only one public function, C<parse>, which expects a stream
as returned by a lexer:
...
The store object is then in the final step of building the IR. The store
determines the names of the columns on which the actual search will take
place. Attempting to search on an attribute that the current "search class"
...
value ("bar"), and an C<$rbracket>, ("]").  The transformation is returning a new
token with the string "BETWEEN" and two normalized values.

As of this writing, the normalized value function merely examines the values to see
if they match a C<Kinetic::DateTime::Incomplete> string and, if they do, return
...
C<Kinetic::Store> object that was passed in to validate that the search can
actually be performed on the Kinetic class on which we are currently searching.
