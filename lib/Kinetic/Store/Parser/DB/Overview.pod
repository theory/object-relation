...
Kinetic::Store::Parser::DB::Overview - Developer's overview of 
Kinetic::Store::Parser::DB

=head1 Synopsis

See L<Kinetic::Store::Parser::DB|Kinetic::Store::Parser::DB>.
...
C<Kinetic::Store::Parser::DB> class operates.

=head1 Grammar

The C<Kinetic::Store::Parser::DB> uses the Parser module described in
"Higher Order Perl" by Mark-Jason Dominus to transform a series of tokens
into an intermediate representation the C<Kinetic::Store> subclasses can
use to initiate searches against a datastore.
...
C<%RE> hash used in I<search_value> are from C<Regexp::Common>.
...
Breaking code into a serious of discrete tokens.  Whitespace is often
discarded.
...
suits a given task.  This structure is often referred to as an 
I<intermediate representation>, or I<IR>.  This document will briefly discuss 
the IR created by C<Kinetic::Store::Parser::DB> and how it's created.

=back

=head1 Lexing

The Kinetic lexers C<Kinetic:Store::Lexer::Code> and
C<Kinetic::Store::Lexer::String> both combine the tokenizing and lexing into a
single step.  See C<Kinetic::Store::Lexer::Overview> for details.

This module has only one public function, C<parse>.  This function expects a 
stream that a lexer returns:

  my $stream = code_lexer_stream($search_request);

And the store object.  It then will either throw a 
C<Kinetic::Util::Exception::Fatal::Search> exception or return an IR.

  my $ir = parse($stream, $store);

The store object is when in the final step of building the IR.  The store is
used to determine the names of the columns on which the actual search will
take place.  Attempting to search on an attribute the current "search class"
does not have will throw an exception.

In the grammar above, the LHS symbols usually represent small "sub-parsers",
each capable of handling the parsing of the RHS.  Thus, the first step in parse
is to pass the input stream to the C<$entire_input> parser:
...
which transforms the input (C<T> being short for I<transform>).  In this case,
it's telling us that we merely wish to return the parse for the statements and
not the C<End_of_Input>.

Now let's take a look at a more complicated example.  Here's the section of
the grammar for "between" values.
...
value ("bar") and a C<$rbracket>, ("]").  The transformation is returning a new
token with the string "BETWEEN" and two normalized values.

The normalized value function, at this time, merely examines the values to see
if thay match a C<Kinetic::DateTime::Incomplete> string and, if they do, return
an object representing them.

=head1 Building search objects

It's not until the parser reaches the "search" LHS bit of the grammar that it
has enough information to create a C<Kinetic::Store::Search> object.
...
C<Kinetic::Store> object which was passed in to validate that the search can
actually be performed on the Kinetic class we are currently searching on.
