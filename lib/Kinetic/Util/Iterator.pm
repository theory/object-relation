...
The returned request will be a hash reference with the keys being the column
names searched for and the values being the search object which corresponds to
the given column.
...
sub request {
    ...
    if (@_) {
        my $ir = shift;
        $self->{request} =
          { map { blessed $_ && $_->can('column') ? ( $_->column => $_ ) : () }
              @$ir };
        return $self;
    }
    return $self->{request};
