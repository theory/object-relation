...
=cut
    ...
    sub uuid_bin    {
        ...
        $uuid->import(str => shift->uuid);
        return $uuid->export('bin');
    }

    sub uuid_hex {
        (my $uuid = shift->uuid) =~ s/-//g;
        return "0x$uuid";
    }

    sub uuid_base64 {
        MIME::Base64::encode_base64(shift->uuid_bin);
    }
...
This method is only called by L<Kinetic::Store|Kinetic::Store> when preparing
to save an object, so that any preparations for the save can be executed. At
this point, that means that the UUID is set if it is not already set. Other
preparatory activities may be added in the future.
...
sub _save_prep {
    ...
    my $uuid = OSSP::uuid->new;
    $uuid->make('v4');
    return $self->uuid($uuid->export('str'));
}
